...
def process_image_form(db,
                       ...
                       title,
                       videoURL):
    books_ref = db.collection(u"books").document(
        bookID).collection("images").document(imageID)
    image = Image(imageID, description, height, width,
                  targetImageURL, ARImageURLs, links, title, videoURL)
    print(image.to_dict())
    return books_ref.set(image.to_dict())


def process_books_form(db,
                       bookID,
                       coverURL,
                       chatID,
                       expertID,
...
def get_new_image_id(db, bookID):
    ...
    images = db.collection(u"books").document(
        bookID).collection("images").get()
        ...
        if complete_form:
            ...
            if file and allowed_file(file.filename):
                ...
                if video_links[0] != "":
                    ...
                    YouTube(video_links[0]).streams.filter(progressive=True, file_extension='mp4').first(
                    ).download("static/videos/", filename=ending)
                    video_links[0] = os.path.join(
                        URL_PREFIX, "static/videos/{}.mp4".format(ending))

                process_image_form(db, book_id, str(image_id), str(description), float(image_height), float(
                    image_width), str(image_url), str(image_links[0]), str(info_links[0]), image_name, str(video_links[0]))
